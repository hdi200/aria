...
TEST_F(Engraving_PartsTests, packagedExcerptsStayInitialized)
{
    MasterScore* score = ScoreRW::readScore(PARTS_DATA_DIR + u"input-from-parts.mscz");
    ASSERT_TRUE(score);
    ASSERT_FALSE(score->excerpts().empty());

    Excerpt* excerpt = score->excerpts().front();
    ASSERT_TRUE(excerpt);
    ASSERT_TRUE(excerpt->inited());

    Score* partScore = excerpt->excerptScore();
    ASSERT_TRUE(partScore);
    Measure* firstMeasure = partScore->firstMeasure();
    ASSERT_TRUE(firstMeasure);

    LayoutBreak* lineBreak = Factory::createLayoutBreak(firstMeasure);
    lineBreak->setLayoutBreakType(LayoutBreakType::LINE);
    firstMeasure->add(lineBreak);
    EXPECT_TRUE(firstMeasure->lineBreak());

    score->initExcerpt(excerpt);

    EXPECT_EQ(excerpt->excerptScore(), partScore);
    ASSERT_TRUE(partScore->firstMeasure());
    EXPECT_TRUE(partScore->firstMeasure()->lineBreak());
