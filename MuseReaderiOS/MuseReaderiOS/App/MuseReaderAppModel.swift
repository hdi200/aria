...
final class MuseReaderAppModel: ObservableObject {
    ...
    func refreshLibraryPreviewAfterClosing(_ session: ScoreSession) async {
        ...
        if let liveRenderSession = session.liveRenderSession, session.capabilities.supportsEditing {
            try? await saveSession(liveRenderSession, to: session.document.url)
        }

        print("Aria close preview refresh: file=\(session.document.url.lastPathComponent)")
        await refreshRenderedLibraryPreviewIfPossible(
            for: session,
            libraryRelativePath: libraryRelativePath
        )
                                     ...
                                     replacingFileReference: String? = nil) async throws -> ScoreSession
    {
        ...
        Task {
            print("Aria library thumbnail task scheduled: file=\(url.lastPathComponent)")
            await refreshRenderedLibraryPreviewIfPossible(
                for: session,
                libraryRelativePath: libraryRelativePath,
                replacingFileReference: replacingFileReference
    ...
    private func refreshRenderedLibraryPreviewIfPossible(for session: ScoreSession,
                                                         libraryRelativePath: String,
                                                         replacingFileReference: String? = nil) async
    {
        guard
        ...
        do {
            ...
            print(String(format: "Aria library thumbnail render finished: file=%@ bytes=%d elapsed=%.3fs", document.url.lastPathComponent, renderedPage.imageData.count, Date().timeIntervalSince(startedAt)))

            recents = recentStore.record(
                document: document,
                libraryRelativePath: libraryRelativePath,
                previewImageData: renderedPage.imageData,
                replacingFileReference: replacingFileReference,
                in: recents
            )
        } catch {
