...
extension ScoreReaderState {
    ...
    func resetToFullScoreBeforeClosing() async {
        guard let liveRenderSession = session.liveRenderSession else {
            return
        }
        ...
        do {
            let updatedPageCount = try await liveRenderSession.setFullScoreView()
            activeScorePartIndex = nil
    ...
    func invalidateRenderedPages(scope: ScoreEditRefreshScope = .all, focusedPageIndex: Int? = nil) {
        ...
        case .all:
            var livePageIndices = Set(
                cachedPagesByIndex.compactMap { index, page in
                    page.source == .liveMuseScoreRenderer ? index : nil
                }
            )
            if isValid(focusPageIndex) {
                livePageIndices.insert(focusPageIndex)
            }
            indicesToInvalidate = livePageIndices
        }
