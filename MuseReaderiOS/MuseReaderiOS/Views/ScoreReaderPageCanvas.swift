...
struct ScoreReaderPageCanvas: View {
    ...
    var body: some View {
        let pageWidth = min(max(availableWidth - reservedRightGutter, 320), 980)
    ...
    private var reservedRightGutter: CGFloat {
    ...
    private var pageAlignment: Alignment {
        ...
        return isPortrait ? .leading : .center
    }
    ...
    private func preferredHeight(for width: CGFloat) -> CGFloat {
        guard let imageSize = page?.image?.size, imageSize.width > 0 else {
...
struct ScoreReaderPageSurface: View {
    ...
    var body: some View {
        ZStack(alignment: .topTrailing) {
            ...
            if let selectedElement, selectedElement.kind == .dynamic {
...
struct ScoreReaderSelectedSignatureActions: View {
    ...
    private func actionPosition(in size: CGSize) -> CGPoint {
        let rect = selectedElement.normalizedRect
        let rawX = CGFloat(rect.x + rect.width) * size.width + 28
        let rawY = CGFloat(rect.y) * size.height - 18
        return CGPoint(
...
struct ScoreReaderSelectionCommandOverlay: View {
    ...
    private func menuPosition(in size: CGSize) -> CGPoint {
        let menuSize = isNoteRestSelection
            ? CGSize(width: 218, height: isNoteSelection ? 392 : 216)
...
private struct ScoreReaderTransposePanel: View {
