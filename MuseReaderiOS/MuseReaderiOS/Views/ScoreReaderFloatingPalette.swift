...
struct ScoreReaderFloatingToolPalette: View {
    ...
    private var dockSideToggle: some View {
        VStack(spacing: 5) {
            ScoreReaderPaletteSeparator()

            Button(action: toggleDockSideAction) {
                Image(systemName: isDockedLeft ? "arrow.right" : "arrow.left")
            ...
            .buttonStyle(.plain)
            .accessibilityLabel(isDockedLeft ? "Move palette right" : "Move palette left")
        }
    }
    ...
    private var noteTools: some View {
        ...
        ScoreReaderPaletteButton(textSymbol: "♮", title: "Natural", isSelected: selectedAccidental == .natural, isEnabled: canEditPitch, action: {
            if editingState.noteInputEnabled {
                ...
                changeAccidentalAction(.natural)
