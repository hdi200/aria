...
struct ScoreReaderWidePitchKeyboard: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            ZStack(alignment: .topLeading) {
                ...
                ForEach(blackKeys, id: \.midiPitch) { key in
                    ZStack(alignment: .bottom) {
                        ...
                        if isActive(midiPitch: key.midiPitch), activeMIDIPitch != nil {
            ...
            .onChange(of: followsActiveMIDIPitch) { _, newValue in
                guard newValue else {
                    return
                }
                updateLockedRange(for: activeMIDIPitch, visibleNaturalKeyCount: visibleNaturalKeyCount)
            }
            .onChange(of: activePitchClass) { _, newValue in
