...
struct ScoreReaderNoteEntrySurface: View {
    ...
    let openInstrumentLayoutAction: () -> Void
    ...
    let changeAccidentalAction: (ScoreAccidentalKind) -> Void
    let removeChordPitchAction: (Int) -> Void
    ...
    private var pitchKeyboardRow: some View {
        HStack(alignment: .bottom, spacing: 0) {
            ...
            ScoreReaderWidePitchKeyboard(
                ...
                activeMIDIPitches: activeChordMIDIPitches,
    ...
    private var detailToolbar: some View {
        ...
        case .layout:
            ScoreReaderLayoutContextToolbar(
                ...
                canOpenGlobalLayoutSettings: !isBusy,
                ...
                openInstrumentLayoutAction: openInstrumentLayoutAction,
    ...
    private var activeChordMIDIPitches: [Int] {
        guard stackedChordInputEnabled else {
            return []
        }

        if let chordMidiPitches = editingState.selection?.chordMidiPitches, !chordMidiPitches.isEmpty {
            return chordMidiPitches
        }

        return pendingMIDIPitch.map { [$0] } ?? []
    }

    private func handlePitchKeyboardTap(_ key: PianoKeyboardKey) {
        if stackedChordInputEnabled && activeChordMIDIPitches.contains(key.midiPitch) {
            removeChordPitchAction(key.midiPitch)
        } else {
