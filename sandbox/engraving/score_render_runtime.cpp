...
{
    const mu::engraving::RectF boundingRect = item->pageBoundingRect();
                                                         ...
                                                         const mu::engraving::PointF& point)
{
    ...
    return selectBestCandidate(page->items(point), point);
}
                                                         ...
                                                         const double maxEdgeDistance)
{
    ...
    for (mu::engraving::EngravingItem* item : page->items(rectAroundPoint(point, selectionRadius))) {
        if (!isDirectlySelectable(item)) {
            continue;
        }
...
bool selectedSingleMeasureRange(const mu::engraving::Score* score,
                                ...
                                mu::engraving::staff_idx_t* staffEnd)
{
    ...
    if (!selectedMeasureRange(score, &startMeasure, &endMeasure) || !startMeasure || startMeasure != endMeasure) {
        return false;
    }
...
double chordRestSegmentEndBoundaryPageX(const mu::engraving::Segment* segment)
{
    const mu::engraving::Measure* measure = segment ? segment->measure() : nullptr;
    if (!measure) {
        return 0.0;
    ...
    const mu::engraving::RectF measureRect = measure->pageBoundingRect();
    const mu::engraving::Segment* nextSegment = segment->next1(mu::engraving::SegmentType::ChordRest);
    if (!nextSegment || nextSegment->measure() != measure) {
        return measureRect.right();
    }

    return std::clamp(
        (chordRestSegmentPageX(segment) + chordRestSegmentPageX(nextSegment)) * 0.5,
        measureRect.left(),
        measureRect.right()
    );
}
...
mu::engraving::Segment* chordRestSegmentAtPointInMeasure(mu::engraving::Measure* measure, const mu::engraving::PointF& pagePoint)
{
    if (!measure) {
        return nullptr;
    }

    mu::engraving::Segment* fallbackSegment = nullptr;
    for (mu::engraving::Segment* segment = measure->first(mu::engraving::SegmentType::ChordRest);
         segment;
         segment = segment->next(mu::engraving::SegmentType::ChordRest)) {
        ...
        const double right = chordRestSegmentEndBoundaryPageX(segment);
        if (pagePoint.x() <= right) {
            return segment;
        }
    }
...
mu::engraving::Segment* chordRestSegmentEndingAtOrBeforePointInMeasure(mu::engraving::Measure* measure,
                                                                       const mu::engraving::PointF& pagePoint)
{
    if (!measure) {
        return nullptr;
    ...
    mu::engraving::Segment* bestSegment = nullptr;
    for (mu::engraving::Segment* segment = measure->first(mu::engraving::SegmentType::ChordRest);
         segment;
         segment = segment->next(mu::engraving::SegmentType::ChordRest)) {
        if (segment->measure() != measure) {
        ...
        if (chordRestSegmentEndBoundaryPageX(segment) <= pagePoint.x() + 0.001) {
            bestSegment = segment;
        } else {
            break;
        }
    ...
    return bestSegment ? bestSegment : measure->first(mu::engraving::SegmentType::ChordRest);
}

void selectSegmentRange(mu::engraving::Score* score,
                        mu::engraving::Segment* startSegment,
                        mu::engraving::Segment* endSegment,
                        const mu::engraving::staff_idx_t staffStartIdx,
                        const mu::engraving::staff_idx_t staffEndIdx)
{
    if (!score || !startSegment || !endSegment) {
        return;
    }
    ...
    mu::engraving::Segment* selectionEnd = endSegment->next1(mu::engraving::SegmentType::ChordRest);
    score->deselectAll();
    const mu::engraving::staff_idx_t staffStart = std::min(staffStartIdx, score->nstaves() - 1);
    const mu::engraving::staff_idx_t staffEnd = std::clamp(
        staffEndIdx,
    ...
    score->selection().setRange(startSegment, selectionEnd, staffStart, staffEnd);
    score->selection().updateSelectedElements();
    score->selection().setActiveTrack(staffStart * mu::engraving::VOICES);
    if (mu::engraving::ChordRest* chordRest = firstStandardChordRestInMeasure(score, startSegment->measure(), staffStart)) {
        score->inputState().setTrack(chordRest->track());
        score->inputState().moveInputPos(startSegment);
    }
    score->setUpdateAll();
}
...
mu::engraving::Measure* activeLayoutBreakMeasure(mu::engraving::Score* score)
{
    mu::engraving::Measure* rangeStart = nullptr;
    mu::engraving::Measure* rangeEnd = nullptr;
    if (selectedMeasureRange(score, &rangeStart, &rangeEnd) && rangeStart) {
        return rangeStart->prevMeasure() ? rangeStart->prevMeasure() : rangeStart;
    }

    return activeMeasure(score);
}
                                   ...
                                   std::string& errorMessage)
{
    ...
    bool accidentalError = false;
    mu::engraving::AccidentalVal accidentalOffset = chord->measure()->findAccidental(
        chord->segment(), chord->vStaffIdx(), bestLine, accidentalError);
    if (accidentalError) {
        accidentalOffset = mu::engraving::Accidental::subtype2value(mu::engraving::AccidentalType::NONE);
    }
                                    ...
                                    std::string& errorMessage)
{
    ...
    fluid_settings_setint(settings.get(), "synth.midi-channels", 16);
