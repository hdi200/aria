...
int main(int argc, char* argv[])
{
    ...
    for (const auto& page : document.pages) {
        const QString fileName = QString("page-%1.png").arg(page.pageIndex + 1, 3, 10, QLatin1Char('0'));
        const muse::io::path_t outputPath = outputDir.appendingComponent(fileName);

        QFile file(outputPath.toQString());
        if (!file.open(QIODevice::WriteOnly) || file.write(reinterpret_cast<const char*>(page.pngData.data()), static_cast<qint64>(page.pngData.size())) == -1) {
            qCritical() << "Failed to save page image:" << outputPath.toQString();
            return 1;
        }
